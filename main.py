...
def capture_from_mic(recogniser, mic_input, application_dic):
    ...
    try:
        ...
        print(application_dic)

        # Read key and value from dic
        for k, val in application_dic.items():
            if audio_recognised.lower() == val:
                print('Opening ', k)
                process = subprocess.Popen(k)
...
def read_application_file():
    ...
    with open(file) as f:
        app_list_dic = dict(x.rstrip().split(None, 1) for x in f)
...
def main():
    ...
    application_dic = read_application_file()
    ...
    audio_recognised = capture_from_mic(recogniser, mic, application_dic)
